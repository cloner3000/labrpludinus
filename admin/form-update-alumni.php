<?php 
	...
	include "koneksi.php";
	$id=$_GET['id'];
	$ambil = "SELECT * FROM alumni WHERE id=$id";
	$query = mysqli_query($koneksi, $ambil);
	while($pecah=mysqli_fetch_array($query)){
		if (mysqli_num_rows($query)>0) {
			$nama=$pecah['nama'];
			$ttl=$pecah['tempat_tgl_lahir'];
			$jurusan=$pecah['jurusan'];
			$tahun_lulus=$pecah['tahun_lulus'];
			$email=$pecah['email'];
		}
	} 
?>
...
font-size: 16px;"><a href="login.php" class="btn btn-danger square-btn-adjust">Logout</a> </div>
        ...
        <div id="page-wrapper" >
            <div id="page-inner">
				<h5>Form Update Alumni</h5>
				<form action="proses-update-alumni.php" method="post">
					<input type="hidden" name="id" value="<?php echo $id ?>">
					  <div class="form-group">
					    <label for="exampleInputEmail1">Nama</label>
					    <input type="text" class="form-control" name="nama" value="<?php echo $nama ?>" aria-describedby="emailHelp" placeholder="Enter nama" >
					  </div>
					  <div class="form-group">
					    <label for="exampleInputPassword1">Tempat, Tanggal Lahir</label>
					    <input type="text" class="form-control" name="tempat_tgl_lahir" value="<?php echo $ttl ?>" placeholder="Tempat, Tanggal Lahir">
					  </div>
					  <div class="form-group">
					    <label for="exampleInputPassword1">Jurusan</label>
					    <input type="text" class="form-control" name="jurusan" value="<?php echo $jurusan ?>" placeholder="Jurusan">
					  </div>
					  <div class="form-group">
					    <label for="exampleInputPassword1">Tahun Lulus</label>
					    <input type="text" class="form-control" name="tahun_lulus" value="<?php echo $tahun_lulus ?>" placeholder="Tahun Lulus">
					  </div>
					  <div class="form-group">
