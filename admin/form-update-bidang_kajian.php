<?php 
  ...
  session_start();
	...
	include "koneksi.php";
	$id=$_GET['id'];
	$ambil = "SELECT * FROM bidang_kajian WHERE id=$id";
	$query = mysqli_query($koneksi, $ambil);
	while($pecah=mysqli_fetch_array($query)){
		if (mysqli_num_rows($query)>0) {
			$tahun=$pecah['tahun'];
      $bidang_kajian=$pecah['bidang_kajian'];
		}
	} 
?>
...
font-size: 16px;"><a href="login.php" class="btn btn-danger square-btn-adjust">Logout</a> </div>
        ...
        <div id="page-wrapper" >
            <div id="page-inner">
				<h5>Form Update Bidang Kajian</h5>
				<form action="proses-update-bidang_kajian.php" method="post">
					<input type="hidden" name="id" value="<?php echo $id ?>">
					  <div class="form-group">
					    <label for="exampleInputEmail1">Tahun</label>
					    <input type="text" class="form-control" name="judul" value="<?php echo $tahun ?>" aria-describedby="emailHelp" placeholder="Tahun" >
					  </div>
					  <div class="form-group">
					    <label for="exampleInputPassword1">Bidang Kajian</label>
					    <input type="text" class="form-control" class="text-center" name="bidang_kajian" value="<?php echo $bidang_kajian ?>" placeholder="Bidang Kajian">
					  </div>
